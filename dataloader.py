...
def load_data(data_path, train_portion=0.8, rand_neg=False, batch_size=32, num_cpu=0, device="cuda"):
    """
    Load all data
    ...
    train_num = int(len(pos_data)*train_portion)
    val_num = int(len(pos_data)*(1-train_portion)*0.5)
    split_size = [train_num, val_num, len(pos_data) - train_num - val_num]
    test_num = len(pos_data) - train_num - val_num
    ic(data_path)
    ic(train_num)
    ic(val_num)
    ic(test_num)
